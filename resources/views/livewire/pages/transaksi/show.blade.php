    ...
    <div class="bg-white rounded-xl shadow p-6 mb-8">
        <div class="mb-4 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
            ...
            <div>
                ...
                <span class="px-3 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-700">{{ $transaksi->status }}</span>
            </div>
        </div>
        <div class="mb-4">
            ...
            <ul class="divide-y divide-slate-100">
                ...
                <li class="py-2 flex justify-between items-center">
                    <span>{{ $item->produk->nama_produk }} <span class="text-xs text-slate-400">x{{ $item->jumlah }}</span></span>
        ...
        <div class="mb-4">
            ...
            <div class="text-base text-indigo-700 font-semibold">Rp {{ number_format($transaksi->total_harga,0,',','.') }}</div>
        </div>
        @if($transaksi->customRequest)
        <div class="mb-4 bg-indigo-50 border-l-4 border-indigo-300 p-4 rounded">
            <div class="font-semibold text-indigo-700 mb-1 flex items-center gap-2"><i class="fa-solid fa-gem"></i> Custom Request</div>
            <div class="text-slate-700">{{ $transaksi->customRequest->deskripsi }}</div>
            <div class="text-xs text-slate-500 mt-1">Estimasi Harga: <span class="text-indigo-700 font-semibold">Rp {{ number_format($transaksi->customRequest->estimasi_harga,0,',','.') }}</span></div>
        </div>
        @endif
        @if($transaksi->pengiriman)
        <div class="mb-4 bg-blue-50 border-l-4 border-blue-300 p-4 rounded">
            <div class="font-semibold text-blue-700 mb-1 flex items-center gap-2"><i class="fa-solid fa-truck-fast"></i> Pengiriman</div>
            <div class="text-slate-700">Status: {{ $transaksi->pengiriman->status }}</div>
            <div class="text-xs text-slate-500 mt-1">Tanggal Pengiriman: {{ $transaksi->pengiriman->tanggal_pengiriman ? $transaksi->pengiriman->tanggal_pengiriman->format('d M Y H:i') : '-' }}</div>
        </div>
        @endif
        <div class="mt-6 flex gap-4">
            <a href="{{ route('transaksi.index') }}" class="px-4 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium flex items-center gap-2"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
        </div>
    </div>
</div>
